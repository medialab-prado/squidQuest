...
//----------------------------------
void sendOSCValue(float val2send){
    /* in the following different ways of creating osc messages are shown by example */
  OscMessage myMessage = new OscMessage("/test");
  
  myMessage.add(val2send); /* add an int to the osc message */
...
//----------------------------------------------
void updateAvgLastValues(float newValueInto) {
  float lerp = 0.95; //must be between 0-1
  quickanddirty = lerp * quickanddirty + (1.0-lerp) * newValueInto;

  text("Quick & dirty Mapped Avg = "+ quickanddirty, 10, height/2);

  //Then send the result by OSC
  
  //println("count: " + count + " new value: " +newValueFromStream + " proper average: " + ave + " quick and dirty: " + quickanddirty);
}
  ...
  // draw the logarithmic averages
  {
    ...
    for(int i = 0; i < fftLog.avgSize(); i++)
    {
      ...
      if ( mouseX >= xl && mouseX < xr )
      {
        ...
        //get selected Freq and apply quick avg
        float miResultadoNormalizado = map(fftLog.getAvg(i), 0, 50, 0, 1);//normalized means  convert the val between 0 and 1. 
        updateAvgLastValues(miResultadoNormalizado); // que guarda el resultado en quickanddirty
        
        //send quickanddirty by OSC to another processing
        sendOSCValue(quickanddirty);
